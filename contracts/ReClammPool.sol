...
contract ReClammPool is IReClammPool, BalancerPoolToken, PoolInfo, BasePoolAuthentication, Version, BaseHooks {
    ...
    ********************************************************/
    ...
    function onBeforeRemoveLiquidity(
        ...
        //
        // Note: when a proportional remove follows an add in the same Vault.unlock() session, the Vault charges a
        // round-trip fee on the remove outputs. This is an intentional Vault-level guardrail: adding and removing
        // in the same session is not something a legitimate user would normally do, and the fee helps ensure the
        // round trip is not profitable. This hook commits virtual balances before that fee is applied, so the
        // stored VBs will be slightly lower than a perfect proportional scaling of the post-fee real balances.
        // The effect is small (bounded by swapFeePercentage * proportionRemoved) and leaves the pool with slightly
        // more real balance relative to its virtual balances, marginally improving centeredness.
