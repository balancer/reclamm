...
describe('ReClammMath', function () {
  ...
  context('isPoolWithinMargins', () => {
    it('centeredness >= centerednessMargin', async () => {
      const balances = [bn(100e18), bn(100e18)];
      const virtualBalances = [bn(90e18), bn(100e18)];

      const res = await mathLib.isPoolWithinMargins(balances, virtualBalances, CENTEREDNESS_MARGIN);
      expect(res).to.equal(isPoolWithinMargins(balances, virtualBalances, CENTEREDNESS_MARGIN));
      expect(res).to.equal(true);
    });
  ...
  context('computeCurrentVirtualBalances', () => {
    ...
    it('q is updating & isPoolWithinMargins == true && lastTimestamp < startTime', async () => {
      ...
      expect(
        await mathLib.isPoolWithinMargins(balancesScaled18, contractVirtualBalances.virtualBalances, CENTEREDNESS_MARGIN)
      ).to.equal(true);
    });
    ...
    it('q is updating & isPoolWithinMargins == true && lastTimestamp > startTime', async () => {
      ...
      expect(await mathLib.isPoolWithinMargins(balancesScaled18, res.virtualBalances, CENTEREDNESS_MARGIN)).to.equal(true);
    });

    it('q is not updating & isPoolWithinMargins == false && isAboveCenter == true', async () => {
      const priceRatioState = await getPriceRatioState();
      ...
      expect(await mathLib.isPoolWithinMargins(balancesScaled18, lastVirtualBalances, CENTEREDNESS_MARGIN)).to.equal(false);
    ...
    it('q is not updating & isPoolWithinMargins == false && isAboveCenter == false', async () => {
      const priceRatioState = await getPriceRatioState();
